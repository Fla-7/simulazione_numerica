...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    double E=2.*(_J*(_particle(this->pbc(i+1)).getspin()+_particle(this->pbc(i-1)).getspin())+_H);

    if(_rnd.Rannyu()<1./(1.+exp(-_beta*E))) {
...
// TO BE FIXED IN EXERCISE 6
  if(_measure_cv){ 
    double cv=pow(_measurement(_index_tenergy),2); //mi serve per poi calcolare <H^2>
...
void System :: averages(int blk){
  ...
  // TO BE FIXED IN EXERCISE 6 
  if (_measure_cv){
    _global_av(_index_cv) -= _average(_index_cv); //devo sottrarre H^2 dal _global_av(index_cv) che contiene la media progressiva relativa alla capacità termica
    _global_av2(_index_cv) -= _average(_index_cv) * _average(_index_cv);
    average = pow(_beta,2) * (_average(_index_cv) - pow(_average(_index_tenergy),2))*_npart; //trovo la capacità termica del SISTEMA
    _global_av(_index_cv) += average; //calcolo la media progressiva con la grandezza che interessa davvero, ovvero C
