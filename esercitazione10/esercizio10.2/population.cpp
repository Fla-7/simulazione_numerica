...
Population :: Population(int dim, vector<double> x, vector<double> y, Random& rnd){
  Chromosome chrom(dim, x, y);
  for(int i=0; i<N; i++){
    pop.push_back(chrom.swap(rnd));
  }
}
...
do{
  ...
  attempts++;
    if (attempts >= max_attempts) {
        //cerr << "Raggiunto numero massimo di tentativi nel crossover. Uso i genitori." << std::endl;
        return {mom, dad};
    }
...
void Population :: average2(int i, int rank){ //rivedere sia media che errore
  ofstream out("ave_per_gen"+to_string(rank)+".dat", ios:: app);
