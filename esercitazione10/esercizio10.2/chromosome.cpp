...
Chromosome:: Chromosome (int dim, vector<double> x, vector<double> y){
  for(int i=0; i<dim; i++){
    chrom.push_back(City(x.at(i), y.at(i)));
  }

  if(chrom.size()!=dim) cerr<<"cromosoma con numero sbagliato di città"<<endl;
