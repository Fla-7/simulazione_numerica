...
void System :: measure(){ // Measure properties
    ...
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if(bin<_n_bins && bin>=0){
          _measurement(_index_gofr+bin)+=2/ (_npart*_rho*(4./3.)*M_PI*(pow(bin*_bin_size+_bin_size,3)-pow(bin*_bin_size,3)));
        }
        if(dr < _r_cut){
        ...
        int norm = _rho*_npart*(4./3.)*M_PI*(pow(bin*_bin_size + _bin_size,3) - pow(bin*_bin_size,3));
        if (bin < _n_bins && bin>=0){
          _measurement(_index_gofr + bin)+=2./norm;
        } */
      }
    }
  }
  ...
  if (_measure_pressure) { 
   if(_sim_type==1) _measurement[_index_pressure] = _rho * (2.0/3.0) * _temp + (_ptail*_npart + 48.0*virial/3.0)/_volume;
    else _measurement[_index_pressure] = _rho * (2.0/3.0) * kenergy_temp + (_ptail*_npart + 48.0*virial/3.0)/_volume;
...
void System :: averages(int blk){
  ...
  if(_measure_gofr){
   ...
   //stampa<<endl; //lascio spazio tra i dati di un istogramma e quelli del successivo
