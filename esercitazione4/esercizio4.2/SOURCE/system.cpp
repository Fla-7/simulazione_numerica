...
// distribuzione delta che mi serve per 4.2

      double v=sqrt(3*_temp);
      if(i%6==0) {vx(i)=v; vy(i)=0; vz(i)=0;}
      else if(i%6==1){vx(i)=-v; vy(i)=0.; vz(i)=0.;}
      else if(i%6==2){vx(i)=0.; vy(i)=v; vz(i)=0.;}
      else if(i%6==3){vx(i)=0.; vy(i)=-v; vz(i)=0.;}
      else if(i%6==4){vx(i)=0.; vy(i)=0.; vz(i)=v;}
      else {vx(i)=0.; vy(i)=0.; vz(i)=-v;}
    ...
    for (int i=0; i<_npart; i++){
      //togliere scalef -> tolto (alla fine ho lasciato tutto perchè non dovrebbe cambiare)
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    for(int i=0; i<_npart; i++){
      ...
      _particle(i).setposition(0, this->pbc(0.5*_side(0)*x, 0)); //il cristallo occupa metà dello spazio in ogni direzione
...
void System :: measure(){ // Measure properties
  ...
  // POFV ... TO BE FIXED IN EXERCISE 4 (fixed)
  if(_measure_pofv){
    int N=0; //per tenere conto delle particelle che ricadono nell'istogramma
    for(int i=0; i<_npart; i++){
      double velocity=0;
      velocity=sqrt(dot(_particle(i).getvelocity() , _particle(i).getvelocity()));
      
      //devo verificare in che bin cade. Ho 30 bin (da INPUT proprieties.dat), come faccio a sapere la velocità max e min?
      //avro v min =0 e v max= 30 * _bin_size_v

      bin= int(velocity/_bin_size_v); //devo arrotondare a intero per difetto per capire in che bin sono
      if (bin<_n_bins_v){
      _measurement(_index_pofv+bin)+=1; //aggiorno il conteggio in un certo bin  
      N+=1;
      }

   }

   //normalizzo: divido ogni conteggio per il numero di particelle
   for(int i=0; i<_n_bins_v; i++){
    _measurement(_index_pofv+i)/=(double)N*_bin_size_v;
   }
...
void System :: averages(int blk){
  ...
  // POFV //////////////////////////////////////////////////////////////////////
  // TO BE FIXED IN EXERCISE 4 (fixed)
  if(_measure_pofv){
