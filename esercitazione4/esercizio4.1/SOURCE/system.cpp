  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4.0*sqrt(_temp)/(double)_n_bins_v; // TO BE FIXED IN EXERCISE 4 ->la velocità dipende dalla radice di T
        _measure_pofv = true;
...
void System :: measure(){ // Measure properties
  ...
  if(_measure_pofv){
    ...
    double N=0;
    for(int i=0; i<_npart; i++){
      //voglio calcolare il modulo della velocità di ogni particella (migliorare questo calcolo)
      for(int j=0; j<3; j++){
        velocity+=(_particle(i).getvelocity(j) * _particle(i).getvelocity(j));
      }
      velocity=sqrt(velocity);
      
      //devo verificare in che bin cade. Ho 30 bin (da INPUT proprieties.dat), come faccio a sapere la velocità max e min?
      //avrò v min =0 e v max= 30 * _bin_size_v

      int bin= floor(velocity/_bin_size_v); //devo arrotondare a intero per difetto per capire in che bin sono
      
      if (bin<30){
      _measurement(_index_pofv+bin)+=1; //aggiorno il conteggio in un certo bin 
      N+=1; //conto quante particelle ci sono nell'istogramma per poterlo normalizzare 
      }
     velocity=0;
   }
   ...
   for(int i=0; i<30; i++){
    _measurement(_index_pofv+i)=_measurement(_index_pofv+i)/(N*_bin_size_v);
   }
